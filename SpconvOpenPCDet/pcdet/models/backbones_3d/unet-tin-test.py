...
import sys
sys.path.append('C:/Users/kritt/Documents/GitHub/NSTDA/SpconvOpenPCDet/pcdet')
sys.path.append('C:/Users/kritt/Documents/GitHub/NSTDA/SpconvOpenPCDet/pcdet/utils')

# import spconv

from ops import spconv as spconv
import torch
...
# Create the voxel tensor
Voxel = torch.randn(
    10,     # batch size
    100,    # input features (input channel) / Max Num Voxels
    50,     # max points per voxel
    4       # nums feature (x, y, z, r)
)

# Create a placeholder batch_dict
batch_size = Voxel.shape[0]
input_channels = Voxel.shape[1]
voxel_features = Voxel.view(batch_size, -1)
voxel_coords = torch.zeros((1, 4))  # Placeholder for voxel coordinates
voxel_num_points = torch.zeros((batch_size,))  # Placeholder for num_points

# Construct the batch_dict
batch_dict = {
    'batch_size': batch_size,
...
# Create an instance of TinNet
model = TinNet(model_cfg=None, input_channels=input_channels, grid_size=(40,40,1), voxel_size=0.1, point_cloud_range=(-20, -20, -5, 20, 20, 5))

# Perform the forward pass
output_batch_dict = model(batch_dict)
